...
public class DefaultPlaceholderValidator implements PlaceholderValidator {
    ...
    // FIXME: 플레이스홀더 포맷을 자유도를 높게 주면 예외 상황이 발생할 것 같다.
    @Override
    public boolean hasPlaceholders(String template) {
        ...
        Set<String> actual = formatter.extractPlaceholders(template);

        if (expectedPlaceholders.size() != actual.size()) {
            for (String placeholder : actual) {
                placeholder = getPurePlaceholderName(formatter, placeholder);
                expectedPlaceholders.remove(placeholder);
            }
            String format = """
                    ...
                    """;
            throw new IllegalArgumentException(format.formatted(String.join(",", expectedPlaceholders), formatter.getPrefix(), formatter.getSuffix()));
        }
